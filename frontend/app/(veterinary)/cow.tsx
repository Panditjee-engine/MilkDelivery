...
// ── Animal Avatar 
function AnimalAvatar({ animal, size = 52, expanded = false }: { animal: Animal; size?: number; expanded?: boolean }) {
    ...
    if (isMale && !isCalf) { iconName = "paw";         iconColor = "#6B4C8B"; }
    else if (isCalf)        { iconName = "leaf-outline"; iconColor = "#3a7d44"; }
    ...
    return (
        <View style={{
            ...
            borderWidth: 1.5, borderColor: borderClr,
...
function InfoRow({
    ...
    return (
        <View style={s.infoRow}>
            ...
            <Text style={[s.infoValue, highlight && { color: C.dark, fontWeight: "800" }]}>{value}</Text>
        </View>
    );
}
...
function AnimalCard({
    ...
    useEffect(() => {
        Animated.parallel([
            ...
            Animated.spring(translateY, { toValue: 0, delay: index * 50, tension: 90, friction: 12, useNativeDriver: true }),
        ]).start();
    }, []);
    ...
    const rotateStr = chevronRot.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "180deg"] });
    const st        = getStatusProps(animal);
    const isMale    = animal.gender === "Male";
    const gColor    = isMale ? "#1a4a8a" : "#7c3aed";
    const gBg       = isMale ? "#EEF4FF"  : "#FDF4FF";

    const onPressIn  = () => Animated.spring(scale, { toValue: 0.975, useNativeDriver: true, tension: 200, friction: 10 }).start();
    ...
    return (
        <Animated.View style={[s.card, expanded && s.cardExpanded, { opacity, transform: [{ translateY }, { scale }] }]}>
            ...
            >
                ...
                <View style={s.cardInfo}>
                    <View style={s.nameRow}>
                        <Text style={s.cowName} numberOfLines={1}>{animal.name || "Unknown"}</Text>
                    ...
                    <View style={s.metaRow}>
                        ...
                        {animal.age !== undefined && animal.age !== null && (
                            <View style={s.chip}>
                                <Text style={s.chipText}>{animal.age} yr{animal.age !== 1 ? "s" : ""}</Text>
                            </View>
                        )}
                    </View>
                </View>
                ...
                <View style={s.cardRight}>
                    ...
                    <Animated.View style={{ transform: [{ rotate: rotateStr }], marginTop: 4 }}>
                ...
                <View style={s.cardBody}>
                    ...
                    <View style={s.identityBanner}>
                        ...
                        <View style={s.identityInfo}>
                            ...
                            <Text style={s.identityBreed}>{animal.breed || "Breed not specified"}</Text>
                    ...
                    <View style={s.infoBlock}>
                        <InfoRow icon="pricetag-outline"  label="Tag Number"   value={animal.tag_number} />
                        <InfoRow icon="paw-outline"        label="Breed"        value={animal.breed || "—"} />
                        <InfoRow icon="calendar-outline"   label="Age"          value={animal.age !== undefined && animal.age !== null ? `${animal.age} year${animal.age !== 1 ? "s" : ""}` : "—"} />
                        <InfoRow icon="barbell-outline"    label="Weight"       value={animal.weight ? `${animal.weight} kg` : "—"} />
                        <InfoRow icon="male-female-outline" label="Gender"      value={animal.gender || "—"} />
                        <InfoRow icon="layers-outline"     label="Type"         value={animal.type || "—"} />
                    </View>

                    {/* ── Date Info ── */}
                    <SectionHeader title="Dates" icon="calendar-outline" />
                    <View style={s.infoBlock}>
                        <InfoRow icon="gift-outline"       label="Date of Birth"    value={fmtDate(animal.dob)} />
                        <InfoRow icon="cart-outline"       label="Purchase Date"    value={fmtDate(animal.purchase_date)} />
                    </View>

                    {/* ── Financial Info ── */}
                    <SectionHeader title="Financial" icon="cash-outline" />
                    <View style={s.infoBlock}>
                        <InfoRow icon="cash-outline"       label="Purchase Price"   value={fmtPrice(animal.purchase_price)} highlight />
                    </View>
        ...
        <View style={s.screen}>
            ...
            <Animated.View style={[s.header, { height: headerHeight }]}>
                ...
                <View style={s.headerTopRow}>
                    ...
                    <TouchableOpacity style={s.iconBtn} onPress={() => setShowScanner(true)}>
                ...
                <Animated.View style={[
                    ...
                    <View style={s.statItem}>
                        <Text style={[s.statValue, { color: "#86efac" }]}>{activeAnimals}</Text>
                        <Text style={s.statLabel}>Active</Text>
                    </View>
                    <View style={s.statDivider} />
                    <View style={s.statItem}>
                        <Text style={[s.statValue, { color: "#fca5a5" }]}>{soldAnimals}</Text>
...
const s = StyleSheet.create({
    ...
    headerTitle: {
        fontSize: 20, fontWeight: "700", color: "#fff", letterSpacing: 0.4,
    ...
    emptyIconBox:  {
        ...
        borderWidth: 1.5, borderColor: "#EDD8C4",
    },
    ...
    tagPill: {
        ...
        borderWidth: 1, borderColor: "#EDD8C4",
    },
    tagPillText: { fontSize: 11, fontWeight: "800", color: C.dark, letterSpacing: 0.5 },
    ...
    bodyDivider: {
        height: 1.5, backgroundColor: C.card,
    ...
    sectionTitle: {
        ...
        color: C.accent, textTransform: "uppercase", letterSpacing: 0.8,
    },
