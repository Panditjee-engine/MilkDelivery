...
// ── Error-message normalizer (same pattern as the medical screen) ──────────
...
// ── Cow shape as returned by /vet/cows (tag_number, not tag) ────────────────
interface VetCow {
  id: string;
  tag_number: string;
  name?: string;
...
// ─── VetCowSelector ─────────────────────────────────────────────────────────
// Loads via api.vetGetCows() — already branch/location filtered server-side.
// We additionally hide leased-OUT cows here (they're locked for this vet),
// but keep leased-IN cows visible and selectable.
function VetCowSelector({
  ...
  onSelect: (cow: VetCow) => void;
  onClear: () => void;
}) {
  const [modalOpen, setModalOpen] = useState(false);
  const [allCows, setAllCows] = useState<VetCow[]>([]);
  const [visibleCows, setVisibleCows] = useState<VetCow[]>([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);
  const [page, setPage] = useState(0);
  const [hasMore, setHasMore] = useState(false);

  const applyFilter = (cows: VetCow[], q: string) => {
    const selectable = cows.filter((c) => !c.isLeasedOut);
    if (!q.trim()) return selectable;
    const needle = q.toLowerCase();
    return selectable.filter(
      (c) =>
        (c.tag_number || "").toLowerCase().includes(needle) ||
  ...
  const load = async () => {
    ...
    try {
      ...
      const cows: VetCow[] = Array.isArray(data) ? data : [];
      setAllCows(cows);
      const filtered = applyFilter(cows, "");
      setVisibleCows(filtered.slice(0, PAGE_SIZE));
  ...
  useEffect(() => {
    const filtered = applyFilter(allCows, search);
    const nextSlice = filtered.slice(0, (page + 1) * PAGE_SIZE);
  ...
  const loadMore = () => {
    ...
    const nextSlice = filtered.slice(0, (nextPage + 1) * PAGE_SIZE);
    setVisibleCows(nextSlice);
    setPage(nextPage);
    setHasMore(nextSlice.length < filtered.length);
  };
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text style={cs.branchHint}>
                Showing cows at your assigned branch only
              </Text>
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={cs.cowTag}>{item.tag_number}</Text>
                        <Text style={cs.cowName}>
                          {item.name}
                          {item.breed ? ` · ${item.breed}` : ""}
                        </Text>
                        {item.isLeasedIn && (
                          <View style={cs.leaseChip}>
                            <Ionicons
                              name="swap-horizontal-outline"
                              size={9}
                              color="#1a4a8a"
                            />
                            <Text style={cs.leaseChipText}>
                              Leased in
                              {item.lessorFarmName
                                ? ` · ${item.lessorFarmName}`
                                : ""}
                    ...
                    hasMore ? (
                      ...
                      >
                        <Ionicons
                          ...
                          color="#7c3aed"
                        />
                        <Text style={cs.loadMoreText}>Load more</Text>
                      </TouchableOpacity>
                    ) : visibleCows.length > 0 ? (
                      <Text style={cs.endText}>✓ All cows loaded</Text>
                    ) : null
                  }
                  ListEmptyComponent={
                    <View style={cs.emptyWrap}>
                      ...
                      <Text style={cs.emptyText}>
                        No selectable cows at your branch
                      </Text>
                      <Text style={cs.emptySubText}>
                        Leased-out animals are hidden until their lease ends
                      </Text>
                    </View>
                  }
                />
...
function AddModal({
  ...
  useEffect(() => {
    if (visible) setForm({ ...EMPTY_FORM, doctorName: vetName ?? "" });
  }, [visible, vetName]);

  const setF = (k: keyof FormData) => (v: any) =>
    setForm((p) => ({ ...p, [k]: v }));
  const handleCowSelect = (cow: VetCow) =>
    setForm((p) => ({ ...p, cowSrNo: cow.tag_number, cowName: cow.name || "" }));
  ...
  const submit = async () => {
    ...
    try {
      ...
      const created = await api.vetCreateInsemination(payload);
      onAdd(created);
      reset();
    } catch (err: any) {
      Alert.alert(
...
function EditModal({
  ...
  const save = async () => {
    ...
    try {
      ...
      const updated = await api.vetUpdateInsemination(record.id, payload);
      onSave(updated);
      onClose();
    } catch (err: any) {
      Alert.alert(
        "Error",
        getErrorMessage(err, "Failed to update record."),
      );
    } finally {
      setSub(false);
    }
      ...
      >
        <View style={c.topRow}>
          ...
          <View style={{ flex: 1, marginLeft: 10 }}>
            ...
            {isLeasedIn && (
              <Text style={c.leasedInHint}>Leased in</Text>
            )}
            {hasMultipleRecords && (
...
export default function VetInseminationScreen() {
  ...
  // ── Build cow lookup (type/photo/leased-in) for card images ──
  useEffect(() => {
    api
      .vetGetCows()
      .then((data: VetCow[]) => {
        const map: Record<
          string,
          { type: string; photo?: string; isLeasedIn?: boolean }
        > = {};
        (Array.isArray(data) ? data : []).forEach((c) => {
          if (!c.tag_number) return;
          map[c.tag_number] = {
            type: c.type || "mature",
            photo: c.photo,
            isLeasedIn: c.isLeasedIn,
          };
        });
        setCowMap(map);
      })
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await api.vetDeleteInsemination(r.id);
  ...
  const groupedRecords = searchFiltered.reduce<InseminationGroup[]>(
    (groups, record) => {
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <InseminationCard
              ...
              isLeasedIn={cowMap[item.cowSrNo]?.isLeasedIn}
              onEdit={(r) => {
                setEditingRecord(r);
                setEditModal(true);
              }}
...
// ─── Styles (unchanged from admin version, plus a few additions) ──────────
...
const cs = StyleSheet.create({
  ...
  leaseChipText: { fontSize: 9, color: "#1a4a8a", fontWeight: "700" },
