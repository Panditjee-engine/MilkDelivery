  ...
  return (
    <Modal transparent visible={config.visible} animationType="none">
      <Animated.View style={[alertStyles.overlay, { opacity: opacityAnim }]}>
        <Animated.View style={[alertStyles.card, { transform: [{ scale: scaleAnim }] }]}>
          {config.icon && (
            <View style={[alertStyles.iconWrap, { backgroundColor: config.iconBg || '#F0F4FF' }]}>
              <Ionicons name={config.icon as any} size={28} color={config.iconColor || Colors.primary} />
            </View>
          )}
...
const alertStyles = StyleSheet.create({
  ...
  title: { fontSize: 19, fontWeight: '800', color: '#111', marginBottom: 8, textAlign: 'center' },
  ...
  actionDestructive: { backgroundColor: '#FEF2F2', borderWidth: 1.5, borderColor: '#FECACA' },
...
function Toast({ visible, message, type }: { visible: boolean; message: string; type: 'success' | 'error' }) {
  ...
  return (
    <Animated.View style={[toastStyles.wrap, { transform: [{ translateY }], opacity }]}>
      <View style={[toastStyles.toast, type === 'error' ? toastStyles.toastError : toastStyles.toastSuccess]}>
        <Ionicons
          name={type === 'success' ? 'checkmark-circle' : 'alert-circle'}
          size={18}
          color={type === 'success' ? '#16a34a' : '#dc2626'}
        />
        <Text style={[toastStyles.text, type === 'error' && { color: '#dc2626' }]}>{message}</Text>
      </View>
    </Animated.View>
  );
}
...
export default function ProfileScreen() {
  ...
  const toggleOrders = () => {
    ...
    Animated.parallel([
      Animated.spring(expandAnim, { toValue, useNativeDriver: false, tension: 60, friction: 10 }),
      Animated.timing(chevronAnim, { toValue, duration: 250, useNativeDriver: true }),
    ]).start();
  };
    ...
    showAlert({
      ...
      actions: [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            try { await api.deleteVacation(id); fetchData(); showToast('Vacation removed'); }
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.hintText}>Deliveries are automatically skipped on these dates.</Text>
          {vacations.length > 0 ? (
            ...
            <>
              {visibleOrders.map((order, i) => {
                ...
                return (
                  <View
                    key={order.id}
                    style={[styles.orderRow, i < visibleOrders.length - 1 && styles.orderRowBorder]}
                  >
...
const styles = StyleSheet.create({
  ...
  addressPillLabel: { fontSize: 10, color: '#aaa', fontWeight: '600', textTransform: 'uppercase', marginBottom: 3 },
  ...
  dateTab: {
    flex: 1, backgroundColor: '#F8F8FA',
    borderRadius: 14, padding: 13,
    borderWidth: 1.5, borderColor: 'transparent',
  },
  dateTabActive: { borderColor: Colors.primary, backgroundColor: Colors.primary + '08' },
  dateTabLabel: { fontSize: 10, color: '#bbb', fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
