...
import Svg, { Rect, Circle, Path, G, Ellipse, Line, Text as SvgText } from "react-native-svg";
...
const statusConfig = (status: string) => {
  switch (status) {
    case "delivered":       return { color: "#16a34a", bg: "#F0FDF4", border: "#BBF7D0", label: "Delivered",        icon: "checkmark-circle" };
    case "out_for_delivery":return { color: "#d97706", bg: "#FFFBEB", border: "#FDE68A", label: "Out for Delivery", icon: "bicycle" };
    case "cancelled":       return { color: "#dc2626", bg: "#FEF2F2", border: "#FECACA", label: "Cancelled",        icon: "close-circle" };
    default:                return { color: "#6366f1", bg: "#EEF2FF", border: "#C7D2FE", label: status?.replace(/_/g, " ") || "Pending", icon: "time" };
  }
};
...
function BrandHeader() {
  ...
  useEffect(() => {
    // Entrance
    Animated.parallel([
      Animated.spring(leafScale,  { toValue: 1, tension: 60, friction: 7, useNativeDriver: true }),
      Animated.timing(leafRotate, { toValue: 1, duration: 700, easing: Easing.out(Easing.back(2)), useNativeDriver: true }),
      Animated.timing(textOpacity,{ toValue: 1, duration: 500, delay: 200, useNativeDriver: true }),
      Animated.spring(textSlide,  { toValue: 0, tension: 80, friction: 8, delay: 150, useNativeDriver: true }),
    ]).start();
  ...
  const rotate = leafRotate.interpolate({ inputRange: [0, 1], outputRange: ["-45deg", "0deg"] });

  return (
    <View style={brandStyles.wrap}>
      {/* Brand mark */}
      <View style={brandStyles.logoRow}>
        <Animated.View style={[brandStyles.leafWrap, { transform: [{ rotate }, { scale: Animated.multiply(leafScale, pulseScale) }] }]}>
          <LinearGradient colors={[Colors.primary, Colors.primary + "BB"]} style={brandStyles.leafGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
            <Ionicons name="leaf" size={18} color="#fff" />
          </LinearGradient>
        </Animated.View>
        <Animated.Text style={[brandStyles.brandName, { opacity: textOpacity, transform: [{ translateX: textSlide }] }]}>
          GauSatv
        </Animated.Text>
...
const brandStyles = StyleSheet.create({
  ...
  leafWrap: { shadowColor: Colors.primary, shadowOpacity: 0.4, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 4 },
  leafGrad: { width: 38, height: 38, borderRadius: 12, justifyContent: "center", alignItems: "center" },
  brandName: { fontSize: 26, fontWeight: "900", color: "#111", letterSpacing: -1 },
  tagPill: { backgroundColor: Colors.primary + "18", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8, marginLeft: 2 },
  tagText: { fontSize: 9, fontWeight: "800", color: Colors.primary, letterSpacing: 1.2 },
});

// ─── Product row item ─────────────────────────────────────────────────────────
function ProductRow({ product, index, onPress }: { product: any; index: number; onPress: () => void }) {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.spring(slideAnim,   { toValue: 0, tension: 70, friction: 9, delay: index * 80, useNativeDriver: true }),
      Animated.timing(opacityAnim, { toValue: 1, duration: 320, delay: index * 80, useNativeDriver: true }),
    ]).start();
  }, []);
    ...
    <Animated.View style={{ opacity: opacityAnim, transform: [{ translateY: slideAnim }] }}>
      ...
      >
        ...
        <View style={[productRowStyles.imgBox, { backgroundColor: theme.bg }]}>
          {product.image ? (
            <Image source={{ uri: product.image }} style={productRowStyles.img} resizeMode="cover" />
          ) : (
        ...
        <View style={productRowStyles.right}>
          <Text style={[productRowStyles.price, { color: theme.accent }]}>₹{product.price}</Text>
          <View style={[productRowStyles.addBtn, { backgroundColor: theme.accent }]}>
...
const productRowStyles = StyleSheet.create({
  ...
  imgBox: { width: 46, height: 46, borderRadius: 14, justifyContent: "center", alignItems: "center", overflow: "hidden" },
  ...
  addBtn: { width: 26, height: 26, borderRadius: 8, justifyContent: "center", alignItems: "center" },
});
...
export default function CustomerHome() {
  ...
  const walletSlide = walletAnim.interpolate({ inputRange: [0, 1], outputRange: [30, 0] });
  ...
  return (
    <SafeAreaView style={s.container} edges={["top"]}>
      ...
      >
        ...
        {/* ── Wallet Card ── */}
        <Animated.View style={{ opacity: walletAnim, transform: [{ translateY: walletSlide }] }}>
          <TouchableOpacity
            style={s.walletCard}
            ...
            {recentOrder && sc ? (
              <>
                <View style={s.orderTopRow}>
                  <View style={[s.orderStatusPill, { backgroundColor: sc.bg, borderColor: sc.border }]}>
                    <Ionicons name={sc.icon as any} size={12} color={sc.color} />
                    <Text style={[s.orderStatusText, { color: sc.color }]}>{sc.label}</Text>
                  </View>
                  {recentOrder.delivery_date && (
                    <Text style={s.orderDate}>{formatDate(recentOrder.delivery_date)}</Text>
                  )}
                </View>
                {recentOrder.items?.slice(0, 3).map((item: any, i: number) => (
                  <View key={i} style={[s.orderItem, i < Math.min(recentOrder.items.length, 3) - 1 && s.orderItemBorder]}>
                ...
                {recentOrder.items?.length > 3 && (
                  <Text style={s.moreItems}>+{recentOrder.items.length - 3} more items</Text>
                )}
              ...
              <View style={s.emptyState}>
                <View style={s.emptyIconWrap}>
                  <Ionicons name="bag-outline" size={28} color={Colors.primary + "80"} />
                </View>
                <Text style={s.emptyTitle}>No orders yet</Text>
                <Text style={s.emptySubtitle}>Your recent order will appear here</Text>
              </View>
            )}
          </View>
        </View>
          ...
          <View style={s.sectionWrap}>
            ...
            <View style={s.card}>
              {featuredProducts.map((product, i) => (
                <ProductRow key={product.id} product={product} index={i} onPress={() => openModal(product)} />
              ))}
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={s.modalOverlay}>
          <View style={s.modalSheet}>
            ...
            <View style={[s.modalProductHeader, { backgroundColor: modalTheme.bg }]}>
              <View style={[s.modalIconCircle, { backgroundColor: modalTheme.accent + "22" }]}>
                {selectedProduct?.image ? (
                  <Image source={{ uri: selectedProduct.image }} style={s.modalProductImage} />
                ) : (
                  <Ionicons name={modalTheme.icon as any} size={30} color={modalTheme.accent} />
                )}
              </View>
              <View style={{ flex: 1 }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={s.patternGrid}>
                {patterns.map((p) => (
                  <TouchableOpacity
                    key={p.value}
                    style={[s.patternCard, pattern === p.value && { backgroundColor: modalTheme.accent, borderColor: modalTheme.accent }]}
                    onPress={() => setPattern(p.value)}
                  >
                    <Ionicons name={p.icon as any} size={19} color={pattern === p.value ? "#fff" : modalTheme.accent} />
                    <Text style={[s.patternLabel, pattern === p.value && { color: "#fff" }]}>{p.label}</Text>
                    <Text style={[s.patternDesc, pattern === p.value && { color: "rgba(255,255,255,0.7)" }]}>{p.description}</Text>
                  </TouchableOpacity>
                ))}
              </View>
                ...
                <>
                  ...
                  <View style={s.daysRow}>
                    {weekDays.map((day) => (
                      <TouchableOpacity
                        key={day.value}
                        style={[s.dayCircle, customDays.includes(day.value) && { backgroundColor: modalTheme.accent }]}
                        onPress={() => toggleCustomDay(day.value)}
                      >
                        <Text style={[s.dayLabel, customDays.includes(day.value) && { color: "#fff" }]}>{day.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </>
...
function BrandFooter() {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.spring(sceneSlide, { toValue: 0, tension: 50, friction: 9, delay: 500, useNativeDriver: true }),
    ]).start();

    Animated.timing(countAnim, { toValue: 1, duration: 1200, delay: 400, easing: Easing.out(Easing.cubic), useNativeDriver: false }).start();
  }, []);

  const countInterp = countAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 500] });
  ...
  return (
    <Animated.View style={[footerStyles.wrap, { opacity: fadeAnim }]}>
      ...
      <Animated.View style={[footerStyles.sceneWrap, { transform: [{ translateY: sceneSlide }] }]}>
        <Svg width="100%" height={130} viewBox="0 0 400 130">
          ...
          {/* Rolling hills */}
          <Path d="M0 95 Q50 65 100 90 Q150 110 200 85 Q250 60 300 88 Q350 108 400 80 L400 130 L0 130 Z" fill={Colors.primary + "22"} />
          <Path d="M0 105 Q60 80 120 100 Q180 118 240 95 Q300 72 360 100 L400 95 L400 130 L0 130 Z" fill={Colors.primary + "40"} />
...
const footerStyles = StyleSheet.create({
  wrap: { alignItems: "center", paddingTop: 32, paddingBottom: 0, backgroundColor: "#fff", marginTop: 8, marginBottom: 0 },
  trustedBy: { fontSize: 11, fontWeight: "700", color: "#bbb", letterSpacing: 2, marginBottom: 6 },
  statRow: { flexDirection: "row", alignItems: "baseline", gap: 4 },
  statNumber: { fontSize: 52, fontWeight: "900", color: "#E0E0E0", letterSpacing: -2 },
  statSuffix: { fontSize: 28, fontWeight: "900", color: "#E0E0E0", letterSpacing: -1 },
  statSub: { fontSize: 11, fontWeight: "700", color: "#bbb", letterSpacing: 2, marginTop: 4, marginBottom: 16 },
  tagline: { fontSize: 13, color: "#aaa", fontWeight: "500", letterSpacing: 0.4, marginBottom: 24 },
  sceneWrap: { width: "100%", overflow: "hidden" },
  madeIn: { fontSize: 10, color: "#aaa", fontWeight: "600", letterSpacing: 1, paddingTop: 8, paddingBottom: 0 },
});
...
const s = StyleSheet.create({
  ...
  walletCard: {
    ...
    borderRadius: 24,
    overflow: "hidden",
    shadowColor: Colors.primary, shadowOpacity: 0.28, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 8,
  },
  ...
  walletLabel: { fontSize: 11, color: "rgba(255,255,255,0.7)", fontWeight: "500", marginBottom: 2 },
  walletAmount: { fontSize: 26, fontWeight: "900", color: "#fff", letterSpacing: -0.8 },
  walletRight: { alignItems: "flex-end", gap: 8 },
  lowBadge: {
    flexDirection: "row", alignItems: "center", gap: 4,
    backgroundColor: "rgba(245,158,11,0.25)", paddingHorizontal: 9, paddingVertical: 4, borderRadius: 20,
  },
  lowBadgeText: { fontSize: 11, color: "#fcd34d", fontWeight: "700" },
  okBadge: {
    flexDirection: "row", alignItems: "center", gap: 4,
    backgroundColor: "rgba(74,222,128,0.2)", paddingHorizontal: 9, paddingVertical: 4, borderRadius: 20,
  },
  okBadgeText: { fontSize: 11, color: "#4ade80", fontWeight: "700" },
  topUpBtn: {
    backgroundColor: "rgba(255,255,255,0.22)", paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20,
  },
  topUpText: { fontSize: 12, fontWeight: "700", color: "#fff" },

  // Sections
  sectionWrap: { marginHorizontal: 20, marginBottom: 20 },
  sectionTitle: { fontSize: 16, fontWeight: "800", color: "#111", marginBottom: 12, letterSpacing: -0.3 },
  ...
  moreItems: { fontSize: 12, color: "#ccc", fontStyle: "italic", paddingTop: 6 },
  orderTotalRow: {
  ...
  orderTotalAmt: { fontSize: 18, fontWeight: "900", color: Colors.primary, letterSpacing: -0.5 },
  ...
  exploreBtnSub: { fontSize: 11, color: Colors.primary, fontWeight: "600", marginTop: 1 },
  ...
  dragHandle: { width: 36, height: 4, backgroundColor: "#E5E7EB", borderRadius: 2, alignSelf: "center", marginBottom: 16 },
  modalProductHeader: {
    flexDirection: "row", alignItems: "center", gap: 14,
    borderRadius: 18, padding: 14, marginBottom: 4,
  },
  modalIconCircle: { width: 56, height: 56, borderRadius: 16, justifyContent: "center", alignItems: "center", overflow: "hidden" },
  modalProductImage: { width: "100%", height: "100%" },
  modalTitle: { fontSize: 17, fontWeight: "800", color: "#111", letterSpacing: -0.3, flexShrink: 1 },
  ...
  modalLabel: { fontSize: 10, fontWeight: "700", color: "#bbb", letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 12, marginTop: 4 },
  ...
  qtyValue: { fontSize: 22, fontWeight: "800", color: "#111", minWidth: 30, textAlign: "center" },
  patternGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10, marginBottom: 24 },
  patternCard: {
    width: "47%", padding: 14, borderRadius: 16,
    backgroundColor: "#F8F8F8", borderWidth: 1.5, borderColor: "transparent", gap: 5,
  },
