...
const MENU = [
  ...
  // },
  {
    ...
    subtitle: "Insemination, semen and medical",
    icon: "medkit",
    route: "/(admin)/gausevak/breeding-health",
  ...
  },
  {
    ...
    subtitle: "Workers & veterinary",
    icon: "people-circle",
    route: "/(admin)/gausevak/users",
    ...
    const tryFetch = async (attempt: number) => {
      ...
      try {
        const res = await fetch(
          "https://wttr.in/Ghaziabad?format=%t|%h|%w|%C",
          {
...
function useCowStats() {
  const [stats, setStats] = useState<CowStats>({
    total: 0,
    active: 0,
    inactive: 0,
  });
  ...
  useEffect(() => {
    api
      ...
      .then((fetchedCows: any[]) => {
        ...
        const active = fetchedCows.filter(
          (c: any) => c.isActive && !c.isSold,
        ).length;
...
function NotificationModal({
    ...
    return (
      <View key={type} style={nm.section}>
        <View style={[nm.sectionHeader, { borderColor: cfg.border }]}>
          ...
          <Text style={[nm.sectionTitle, { color: cfg.color }]}>
            {cfg.label}
          </Text>
          <View
            style={[
        ...
        {allDone ? (
          ...
          >
            ...
            <Text style={[nm.allDoneText, { color: cfg.color }]}>
              {cfg.doneLabel}
            </Text>
          </View>
        ) : (
          <>
            {cat.pending.length > 0 && (
              <View style={nm.cowGroup}>
                <Text style={nm.cowGroupLabel}>
                  Pending ({cat.total_pending})
                </Text>
                <View style={nm.cowChipsWrap}>
                  {cat.pending.map((name, i) => (
                    ...
                    >
                      <View
                        style={[nm.chipDot, { backgroundColor: cfg.color }]}
                      />
                      <Text style={[nm.chipText, { color: cfg.color }]}>
                        {name}
                      </Text>
                    </View>
                  ))}
            ...
            {cat.done.length > 0 && (
              <View style={nm.cowGroup}>
                <Text style={[nm.cowGroupLabel, { color: "#3a5a3a" }]}>
                  Done ({cat.total_done})
                </Text>
                <View style={nm.cowChipsWrap}>
                  {cat.done.map((name, i) => (
  ...
  return (
    ...
    >
      ...
      <Animated.View
        style={[nm.sheet, { transform: [{ translateY: slideAnim }] }]}
      >
          ...
          <View style={nm.headerLeft}>
            ...
            <View>
              ...
              <Text style={nm.headerSub}>
                {shift === "morning" ? "🌅 Morning" : "🌙 Evening"} shift ·{" "}
                {todayStr()}
              </Text>
            </View>
          </View>
          ...
          <View style={nm.summaryRow}>
            {(["feed", "milk", "health"] as const).map((type) => {
              ...
              return (
                ...
                >
                  <Ionicons
                    name={isDone ? "checkmark-circle" : cfg.icon}
                    size={12}
                    color={isDone ? "#4ade80" : cfg.color}
                  />
                  <Text
                    style={[
                      nm.summaryChipText,
        ...
        {loading ? (
          ...
          <View style={nm.centered}>
            ...
            <Text style={nm.allGoodSub}>Worker Notification</Text>
          </View>
        ) : (
          <ScrollView
            showsVerticalScrollIndicator={false}
            contentContainerStyle={nm.scrollContent}
          >
            {renderSection("feed")}
            {renderSection("milk")}
