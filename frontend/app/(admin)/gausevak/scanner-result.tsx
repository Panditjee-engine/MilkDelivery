...
export default function ScannerResultScreen() {
  ...
  useEffect(() => {
    if (!data) { setLoading(false); setNotFound(true); return; }
    fetchCow();
  }, [data]);
  ...
  useEffect(() => {
    if (!loading) {
      Animated.parallel([
        ...
        Animated.spring(scaleHero, { toValue: 1, tension: 60, friction: 10, useNativeDriver: true }),
      ]).start();
    }
  }, [loading]);
      ...
      <View style={s.screen}>
        ...
        <Animated.View style={[s.centered, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          ...
          <View style={s.scannedRaw}>
            ...
            <Text style={s.scannedRawText} numberOfLines={1}>{data}</Text>
          </View>
          <TouchableOpacity style={s.scanAgainBtn} onPress={() => router.back()}>
  ...
  const statusLabel = cow.isSold ? "Sold" : cow.isActive ? "Active" : "Inactive";
    ...
    <View style={s.screen}>
      ...
      <View style={[s.topBar, { paddingTop: IS_IOS ? 56 : STATUS_H + 16 }]}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push(`/(admin)/gausevak/cows/${cow._id}` as any)}
        >
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        ...
        {/* ── Hero Card ── */}
        <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleHero }] }}>
          <LinearGradient
          ...
          >
            ...
            <View style={s.heroTop}>
              ...
              <View style={[s.heroBadge, { backgroundColor: statusBg }]}>
                ...
                <Text style={[s.heroBadgeText, { color: statusColor }]}>{statusLabel}</Text>
              </View>
            </View>
            ...
            <View style={s.heroTagRow}>
              <View style={s.heroTag}>
                ...
                <Text style={s.heroTagText}>{cow.tag_id || cow.tag || "No Tag"}</Text>
              </View>
              {cow.age && (
                <View style={s.heroTag}>
              ...
              {cow.gender && (
                <View style={s.heroTag}>
                  <Ionicons name="male-female-outline" size={12} color="#b7e4c7" />
            ...
            <View style={s.heroStats}>
              <View style={s.heroStat}>
                <Text style={s.heroStatVal}>{cow.weight ? `${cow.weight}` : "—"}</Text>
                <Text style={s.heroStatLbl}>kg Weight</Text>
              </View>
              <View style={s.heroStatDivider} />
              <View style={s.heroStat}>
                <Text style={s.heroStatVal}>{cow.milk_yield ? `${cow.milk_yield}L` : "—"}</Text>
                <Text style={s.heroStatLbl}>Milk/day</Text>
              </View>
              <View style={s.heroStatDivider} />
              <View style={s.heroStat}>
                <Text style={s.heroStatVal}>{cow.shed || cow.stall || "—"}</Text>
        ...
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          {/* ── Basic Info ── */}
          <SectionCard title="Basic Info" icon="information-circle-outline" color="#2d6a4f">
            <View style={s.chipGrid}>
              <InfoChip icon="paw-outline" label="Name" value={cow.name} color="#2d6a4f" />
              <InfoChip icon="leaf-outline" label="Breed" value={cow.breed} color="#2d6a4f" />
              <InfoChip icon="pricetag-outline" label="Tag ID" value={cow.tag_id || cow.tag} color="#2d6a4f" />
              <InfoChip icon="male-female-outline" label="Gender" value={cow.gender} color="#2d6a4f" />
              <InfoChip icon="calendar-outline" label="Age" value={cow.age ? `${cow.age} years` : cow.dob} color="#2d6a4f" />
              <InfoChip icon="color-palette-outline" label="Color" value={cow.color} color="#2d6a4f" />
            </View>
          </SectionCard>

          {/* ── Health & Production ── */}
          <SectionCard title="Health & Production" icon="heart-outline" color="#7c3aed">
            <View style={s.statPillRow}>
              <StatPill label="Weight" value={cow.weight ? `${cow.weight} kg` : ""} color="#7c3aed" />
              <StatPill label="Milk/day" value={cow.milk_yield ? `${cow.milk_yield} L` : ""} color="#0284c7" />
              <StatPill label="Vaccination" value={cow.vaccination_status} color="#16a34a" />
            </View>
            <View style={s.chipGrid}>
              <InfoChip icon="medkit-outline" label="Last Checkup" value={cow.last_checkup} color="#7c3aed" />
              <InfoChip icon="fitness-outline" label="Health Status" value={cow.health_status} color="#7c3aed" />
            </View>
          </SectionCard>

          {/* ── Farm Details ── */}
          <SectionCard title="Farm Details" icon="location-outline" color="#b45309">
            <View style={s.chipGrid}>
              <InfoChip icon="home-outline" label="Shed / Stall" value={cow.shed || cow.stall} color="#b45309" />
              <InfoChip icon="business-outline" label="Farm" value={cow.farm_name} color="#b45309" />
              <InfoChip icon="time-outline" label="Added On" value={cow.created_at ? new Date(cow.created_at).toLocaleDateString() : ""} color="#b45309" />
            </View>
          </SectionCard>
          ...
          <View style={s.actionsGrid}>
            <TouchableOpacity
              style={s.actionCard}
              onPress={() => router.push(`/(admin)/gausevak/feed?cow_id=${cow._id}` as any)}
            >
            ...
            <TouchableOpacity
              style={s.actionCard}
              onPress={() => router.push(`/(admin)/gausevak/health?cow_id=${cow._id}` as any)}
            >
            ...
            <TouchableOpacity
              style={s.actionCard}
              onPress={() => router.push(`/(admin)/gausevak/milkyield?cow_id=${cow._id}` as any)}
            >
            ...
            <TouchableOpacity
              style={s.actionCard}
              onPress={() => router.push(`/(admin)/gausevak/medical?cow_id=${cow._id}` as any)}
            >
...
const s = StyleSheet.create({
  ...
  topTitle: { fontSize: 16, fontWeight: "800", color: "#0f172a", letterSpacing: -0.3 },
  ...
  notFoundCircle: {
    ...
    backgroundColor: "#fef2f2", alignItems: "center", justifyContent: "center", marginBottom: 8,
  },
  ...
  scannedRaw: {
    ...
    borderWidth: 1, borderColor: "#e2e8f0", marginTop: 4, maxWidth: "90%",
  },
  scannedRawText: {
    color: "#64748b", fontSize: 13, fontWeight: "600",
    fontFamily: Platform.OS === "ios" ? "Courier New" : "monospace", flex: 1,
  },
  scanAgainBtn: {
    flexDirection: "row", alignItems: "center", gap: 8,
  ...
  heroCard: {
    ...
    shadowColor: "#1a472a", shadowOffset: { width: 0, height: 8 },
  ...
  heroCowName: { fontSize: 28, fontWeight: "900", color: "#fff", letterSpacing: -0.5, marginBottom: 2 },
  heroCowBreed: { fontSize: 14, color: "rgba(255,255,255,0.65)", fontWeight: "500", marginBottom: 14 },
  heroTagRow: { flexDirection: "row", gap: 8, flexWrap: "wrap", marginBottom: 18 },
  heroTag: {
    flexDirection: "row", alignItems: "center", gap: 5,
  ...
  heroStats: {
    flexDirection: "row", backgroundColor: "rgba(0,0,0,0.18)", borderRadius: 16, padding: 14,
  },
  heroStat: { flex: 1, alignItems: "center" },
  heroStatDivider: { width: 1, backgroundColor: "rgba(255,255,255,0.15)" },
  heroStatVal: { fontSize: 18, fontWeight: "900", color: "#fff", letterSpacing: -0.3 },
  heroStatLbl: { fontSize: 10, color: "rgba(255,255,255,0.55)", fontWeight: "600", marginTop: 2 },
  ...
  scannedCardLabel: { fontSize: 11, color: "#94a3b8", fontWeight: "600", marginBottom: 2 },
  scannedCardValue: {
  ...
  cardTitle: { fontSize: 12, fontWeight: "800", letterSpacing: 0.5, textTransform: "uppercase" },
  cardBody: { padding: 12 },
  ...
  chipValue: { fontSize: 13, color: "#0f172a", fontWeight: "700", marginTop: 1 },

  // Stat pills
  statPillRow: { flexDirection: "row", gap: 8, marginBottom: 12 },
  statPill: { flex: 1, alignItems: "center", paddingVertical: 12, borderRadius: 14, borderWidth: 1 },
  statPillValue: { fontSize: 16, fontWeight: "900", letterSpacing: -0.3 },
  statPillLabel: { fontSize: 10, color: "#94a3b8", fontWeight: "600", marginTop: 2 },
  ...
  actionCard: {
    ...
    alignItems: "flex-start", borderWidth: 1, borderColor: "#f1f5f9",
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 },
  ...
  actionSub: { fontSize: 11, color: "#94a3b8", fontWeight: "500", marginTop: 2 },
});