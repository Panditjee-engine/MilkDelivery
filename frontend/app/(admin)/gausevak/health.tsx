...
const SEVERITY = {
  ...
  severe:   { label: "Severe",   color: "#dc2626", bg: "#fef2f2", border: "#fca5a5" },
};

// ─── Status helper ────────────────────────────────────────────────────────────
function getOverallStatus(r: HealthRecord) {
  if (r.isHealthy)
    return { label: "Healthy",        color: "#16a34a", bg: "#f0fdf4", border: "#86efac", icon: "checkmark-circle" };
  if (r.checkupDone)
    return { label: "Under Treatment", color: "#7c3aed", bg: "#faf5ff", border: "#e9d5ff", icon: "medical" };
  return   { label: "Needs Checkup",  color: "#dc2626", bg: "#fef2f2", border: "#fca5a5", icon: "alert-circle" };
}
...
// ─── Severity Picker ─────────────────────────────────────────────────────────
function SeverityPicker({ value, onChange }: { value: "mild" | "moderate" | "severe"; onChange: (v: any) => void }) {
  return (
    <View style={f.wrap}>
        ...
        {(["mild", "moderate", "severe"] as const).map((s) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                f.severityChip,
                { borderColor: active ? cfg.color : "#e5e7eb", backgroundColor: active ? cfg.bg : "#f9fafb" },
              ]}
            >
              <View style={[f.severityDot, { backgroundColor: cfg.color }]} />
              <Text style={[f.severityText, { color: active ? cfg.color : "#9ca3af" }]}>{cfg.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
// ─── Add / Edit Modal ─────────────────────────────────────────────────────────
function RecordModal({ visible, record, onClose, onSave }: {
  visible: boolean;
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ width: "100%" }}>
          <View style={mo.sheet}>
            ...
            <View style={mo.header}>
              <View style={[mo.headerIcon, { backgroundColor: isEdit ? "#fff7ed" : "#f0fdf4" }]}>
                <Ionicons name={isEdit ? "create" : "medkit"} size={18} color={isEdit ? "#ea580c" : "#16a34a"} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={mo.title}>{isEdit ? "Update Health Record" : "New Health Record"}</Text>
                {isEdit && record && (
                  <Text style={mo.subName}>{record.cowName} · {record.cowSrNo}</Text>
                )}
              </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 500 }}>
              ...
              <SectionHeader title="Health Issue" icon="medkit-outline" color="#dc2626" />
              <Field label="Health Issue / Disease" value={form.healthIssue} onChange={setF("healthIssue")} placeholder="e.g. Foot and Mouth Disease" icon="bug-outline" />
              <Field label="Issue Date" value={form.issueDate} onChange={setF("issueDate")} placeholder="DD/MM/YYYY" icon="calendar-outline" />
              <SeverityPicker value={form.severity} onChange={setF("severity")} />
              <Field label="Notes" value={form.notes} onChange={setF("notes")} placeholder="Additional observations..." icon="document-text-outline" />

              <SectionHeader title="Checkup Status" icon="stethoscope" color="#7c3aed" />
              <ToggleCard
                label="Checkup Done"
                sub={form.checkupDone ? "Vet has examined the cow ✓" : "Checkup not done yet"}
              ...
              {form.checkupDone && (
                <View style={mo.subFields}>
                  <Field label="Checkup Date" value={form.checkupDate} onChange={setF("checkupDate")} placeholder="DD/MM/YYYY" icon="calendar-outline" />
                  <Field label="Doctor Name" value={form.doctorName} onChange={setF("doctorName")} placeholder="e.g. Dr. Sharma" icon="person-outline" />
                </View>
              )}

              <SectionHeader title="Recovery Status" icon="heart" color="#16a34a" />
              <ToggleCard
                label="Cow is Healthy"
                sub={form.isHealthy ? "Fully recovered ✓" : "Still under observation"}
              ...
              {form.isHealthy && (
                <View style={mo.subFields}>
                  <Field label="Resolved Date" value={form.resolvedDate} onChange={setF("resolvedDate")} placeholder="DD/MM/YYYY" icon="calendar-outline" />
                </View>
              )}
            ...
            <TouchableOpacity
              onPress={handleSave}
              style={[mo.submitBtn, { backgroundColor: isEdit ? "#ea580c" : "#16a34a" }, !canSave && { opacity: 0.45 }]}
              disabled={!canSave}
            >
              <Ionicons name={isEdit ? "save-outline" : "checkmark-circle-outline"} size={18} color="#fff" />
              <Text style={mo.submitText}>{isEdit ? "Update Record" : "Save Record"}</Text>
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </View>
...
function DetailRow({ icon, label, value, color }: { icon: string; label: string; value: string; color?: string }) {
  ...
  return (
    <View style={dr.row}>
      <View style={[dr.iconWrap, { backgroundColor: (color ?? "#6b7280") + "15" }]}>
...
// ─── Health Card ──────────────────────────────────────────────────────────────
function HealthCard({ item, index, onEdit, onQuickToggle }: {
  item: HealthRecord;
  index: number;
  onEdit: (r: HealthRecord) => void;
  onQuickToggle: (id: string, field: "checkupDone" | "isHealthy", value: boolean) => void;
}) {
  ...
  return (
    <Animated.View style={[hc.card, item.isHealthy && hc.cardHealthy, { opacity, transform: [{ translateY }] }]}>
      <TouchableOpacity onPress={() => setExpanded((e) => !e)} activeOpacity={0.8}>
        {/* Top row */}
        <View style={hc.topRow}>
          <View style={[hc.avatar, item.isHealthy && { backgroundColor: "#f0fdf4", borderColor: "#86efac" }]}>
          ...
          <View style={[hc.statusBadge, { backgroundColor: status.bg, borderColor: status.border }]}>
            <Ionicons name={status.icon as any} size={11} color={status.color} />
            <Text style={[hc.statusText, { color: status.color }]}>{status.label}</Text>
          </View>
        ...
        <View style={hc.issueRow}>
          ...
          <Text style={hc.issueText} numberOfLines={1}>{item.healthIssue}</Text>
          <View style={[hc.sevBadge, { backgroundColor: sev.bg, borderColor: sev.border }]}>
        ...
        <View style={hc.quickToggles}>
          <TouchableOpacity
            style={[hc.toggleChip, item.checkupDone && { backgroundColor: "#faf5ff", borderColor: "#c4b5fd" }]}
            onPress={() => onQuickToggle(item.id, "checkupDone", !item.checkupDone)}
            activeOpacity={0.8}
          >
            <Ionicons name={item.checkupDone ? "checkmark-circle" : "ellipse-outline"} size={14} color={item.checkupDone ? "#7c3aed" : "#9ca3af"} />
            <Text style={[hc.toggleChipText, { color: item.checkupDone ? "#7c3aed" : "#9ca3af" }]}>
              {item.checkupDone ? "Checkup Done" : "Mark Checkup"}
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[hc.toggleChip, item.isHealthy && { backgroundColor: "#f0fdf4", borderColor: "#86efac" }]}
            onPress={() => onQuickToggle(item.id, "isHealthy", !item.isHealthy)}
            activeOpacity={0.8}
          >
            <Ionicons name={item.isHealthy ? "heart-circle" : "heart-outline"} size={14} color={item.isHealthy ? "#16a34a" : "#9ca3af"} />
            <Text style={[hc.toggleChipText, { color: item.isHealthy ? "#16a34a" : "#9ca3af" }]}>
      ...
      {expanded && (
        <View>
          ...
          <DetailRow icon="barcode-outline" label="Sr. No." value={item.cowSrNo} color="#2563eb" />

          <Text style={hc.section}>🤒 Health Issue</Text>
          <DetailRow icon="bug-outline" label="Issue" value={item.healthIssue} color="#dc2626" />
          <DetailRow icon="calendar-outline" label="Reported On" value={item.issueDate} color="#dc2626" />
          <DetailRow icon="warning-outline" label="Severity" value={sev.label} color={sev.color} />
          {item.notes ? <DetailRow icon="document-text-outline" label="Notes" value={item.notes} color="#6b7280" /> : null}

          <Text style={hc.section}>🩺 Checkup</Text>
          <DetailRow icon="checkmark-circle-outline" label="Checkup Done" value={item.checkupDone ? "Yes" : "No"} color={item.checkupDone ? "#7c3aed" : "#9ca3af"} />
          {item.checkupDone && (
            <>
              <DetailRow icon="calendar-outline" label="Checkup Date" value={item.checkupDate} color="#7c3aed" />
              <DetailRow icon="person-outline" label="Doctor" value={item.doctorName} color="#7c3aed" />
            </>
          )}

          <Text style={hc.section}>💚 Recovery</Text>
          <DetailRow icon="heart" label="Status" value={item.isHealthy ? "Fully Recovered ✓" : "Under Observation"} color={item.isHealthy ? "#16a34a" : "#9ca3af"} />
          {item.isHealthy && <DetailRow icon="calendar-outline" label="Resolved On" value={item.resolvedDate} color="#16a34a" />}

          {/* Update button */}
          <TouchableOpacity style={hc.updateBtn} onPress={() => onEdit(item)} activeOpacity={0.85}>
...
export default function CowHealthScreen() {
  ...
  const [filter, setFilter] = useState<"all" | "healthy" | "sick" | "pending">("all");
  ...
  const filtered = records.filter((r) => {
    ...
    const matchFilter =
      filter === "all" ? true :
      filter === "healthy" ? r.isHealthy :
      filter === "sick" ? (!r.isHealthy && r.checkupDone) :
      (!r.checkupDone);
  ...
  const openAdd = () => { setEditingRecord(null); setModalVisible(true); };
  const openEdit = (r: HealthRecord) => { setEditingRecord(r); setModalVisible(true); };
  ...
  const handleQuickToggle = (id: string, field: "checkupDone" | "isHealthy", value: boolean) => {
    setRecords((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
  ...
  const FILTERS: { key: typeof filter; label: string; color: string; count: number }[] = [
    ...
    { key: "pending", label: "Pending",  color: "#dc2626", count: stats.pending },
    { key: "sick",    label: "Sick",     color: "#7c3aed", count: stats.sick    },
    { key: "healthy", label: "Healthy",  color: "#16a34a", count: stats.healthy },
  ];
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>
        {FILTERS.map((fl) => (
          <TouchableOpacity
            ...
            style={[s.filterChip, filter === fl.key && { backgroundColor: fl.color, borderColor: fl.color }]}
          >
            <Text style={[s.filterChipText, filter === fl.key && { color: "#fff" }]}>{fl.label}</Text>
            <View style={[s.filterBadge, filter === fl.key && { backgroundColor: "rgba(255,255,255,0.3)" }]}>
              <Text style={[s.filterBadgeText, filter === fl.key && { color: "#fff" }]}>{fl.count}</Text>
            </View>
          </TouchableOpacity>
        ))}
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  addBtn: {
    ...
    backgroundColor: "#16a34a", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
  },
  ...
  fab: {
    ...
    width: 54, height: 54, borderRadius: 27, backgroundColor: "#16a34a",
...
const hc = StyleSheet.create({
  ...
  issueRow: {
    flexDirection: "row", alignItems: "center", gap: 6,
    marginTop: 10, backgroundColor: "#fef2f2", borderRadius: 8,
    paddingHorizontal: 10, paddingVertical: 7,
  },
  issueText: { flex: 1, fontSize: 12, fontWeight: "600", color: "#991b1b" },
  sevBadge: { paddingHorizontal: 7, paddingVertical: 2, borderRadius: 10, borderWidth: 1 },
  ...
  updateBtn: {
    ...
    gap: 7, marginTop: 16, backgroundColor: "#ea580c", borderRadius: 12, paddingVertical: 12,
  },
...
const mo = StyleSheet.create({
  ...
  headerIcon: { width: 34, height: 34, borderRadius: 10, alignItems: "center", justifyContent: "center", marginRight: 10 },
  ...
  subFields: { backgroundColor: "#f9fafb", borderRadius: 14, padding: 12, borderWidth: 1, borderColor: "#e0e7ff", marginBottom: 4 },
  submitBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    borderRadius: 14, paddingVertical: 15, gap: 8, marginTop: 14,
  },
